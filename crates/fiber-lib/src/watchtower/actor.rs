...
pub(crate) fn build_settlement_transaction(
    tx_builder: TransactionBuilder,
    witness_for_commitment_cell: Vec<u8>,
    placeholder_witness_for_change: WitnessArgs,
) -> TransactionBuilder {
    tx_builder
...
fn scan_watched_settlement_txs<S: WatchtowerStore>(
    ...
    loop {
        ...
        for tx in &candidates {
            ...
            if let Some(witness_index) = process_watched_settlement_tx_with_features(
                tx,
...
fn process_watched_settlement_tx<S: WatchtowerStore>(
    ...
    process_watched_settlement_tx_with_features(
        tx,
        watched_outpoints,
        processed_tx_hashes,
        commitment_lock_prefix,
...
#[allow(clippy::too_many_arguments)]
fn process_watched_settlement_tx_with_features<S: WatchtowerStore>(
    tx: &Transaction,
    watched_outpoints: &mut HashMap<OutPoint, Vec<TrackedSettlementTlc>>,
    processed_tx_hashes: &mut HashSet<ckb_types::packed::Byte32>,
    commitment_lock_prefix: &Script,
...
fn build_settlement_tx<S: WatchtowerStore>(
    ...
    tracked_tlcs: &[TrackedSettlementTlc],
    signer: &LocalSigner,
    cell_collector: &mut impl CellCollector,
    store: &S,
    ...
    if cell_output.type_().is_none() {
        ...
        tx_builder = tx_builder
            ...
            .output_data(Bytes::default());
        tx_builder = build_settlement_transaction(
            tx_builder,
            witness_for_commitment_cell,
            placeholder_witness_for_change,
        );
...
mod tests {
    ...
    #[test]
    fn feature_bitmap_watch_channel_registration_reaches_store() {
        let store = TestWatchtowerStore::default();
        let settlement_data = SettlementData {
            local_amount: 1,
            remote_amount: 2,
    ...
    fn test_fresh_channel_pre_tlc_commitment_builds_settlement() {
        ...
        let channel_data = ChannelData {
            channel_id: [9u8; 32].into(),
            funding_udt_type_script: None,
            local_settlement_key: Privkey::from(&[1; 32]),
