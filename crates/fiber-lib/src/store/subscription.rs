...
impl TryFrom<PaymentSessionStatus> for PaymentUpdatedPayload {
    type Error = String;

    fn try_from(value: PaymentSessionStatus) -> Result<Self, Self::Error> {
        let payload = match value {
            PaymentSessionStatus::Created => PaymentUpdatedPayload::Created,
            PaymentSessionStatus::Inflight => PaymentUpdatedPayload::Inflight,
            PaymentSessionStatus::Success => {
                return Err("PaymentUpdatedPayload::Created requires extra data".to_string())
            }
            PaymentSessionStatus::Failed => PaymentUpdatedPayload::Failed,
        };
        Ok(payload)
    }
}
