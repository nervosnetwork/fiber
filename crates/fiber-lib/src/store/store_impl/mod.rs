...
impl PaymentEventStore for Store {
    ...
    fn insert_payment_event(&self, event: PaymentEvent) {
        let mut batch = self.batch();
        let kv = KeyValue::PaymentEvent(event);
        batch.put(kv.key(), kv.value());
        batch.commit();
    }
...
impl InvoiceStore for Store {
    ...
    fn get_invoices_with_limit(
        ...
        let filter_invoice = |kv: &KVPair| -> Option<(CkbInvoice, CkbInvoiceStatus)> {
            let invoice: CkbInvoice = deserialize_from(kv.value.as_ref(), "CkbInvoice");
            // Extract the payment hash from the key (skip the 1-byte prefix)
            if kv.key.len() < 33 {
                return None;
            }
            let hash_bytes: [u8; 32] = kv.key[1..33].try_into().ok()?;
        ...
        match after {
            Some(after_hash) => {
                ...
                self.collect_by_prefix_with(&prefix, options)
                    .into_iter()
                    .filter_map(|ref kv| filter_invoice(kv))
                    .take(limit)
                    .collect()
            }
            None => {
