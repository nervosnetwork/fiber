...
struct TopologyConnectTo {
    id: String,
    targets: Vec<String>,
    capacities: Vec<u128>,
}

fn parse_list_lines<I>(first_line: &str, lines: &mut I) -> String
where
...
fn parse_string_list(list_text: &str) -> Vec<String> {
    let start = list_text.find('[').unwrap_or(0);
    let end = list_text.rfind(']').unwrap_or(list_text.len());
    let inner = &list_text[start + 1..end];
    inner
        .split(',')
...
fn parse_u128_list(list_text: &str) -> Vec<u128> {
    let start = list_text.find('[').unwrap_or(0);
    let end = list_text.rfind(']').unwrap_or(list_text.len());
    let inner = &list_text[start + 1..end];
    inner
...
fn node_type_and_index(id: &str) -> (String, u64) {
    let mut parts = id.split('_');
    let node_type = parts.next().unwrap_or("unknown").to_string();
    let index = parts
        .next()
...
fn node_type_order(node_type: &str) -> u64 {
    match node_type {
        "core" => 0,
        "relay" => 1,
        "edge" => 2,
...
fn should_include_node(id: &str, allow_types: &[String]) -> bool {
    let (node_type, _) = node_type_and_index(id);
    allow_types.iter().any(|t| t == &node_type)
}

fn capacity_to_amount(capacity: u128) -> u128 {
    MIN_RESERVED_CKB + capacity.saturating_mul(1000)
}

fn parse_connect_to_blocks(contents: &str) -> Vec<TopologyConnectTo> {
    let mut connects = Vec::new();
    let mut current = TopologyConnectTo::default();
    let mut in_block = false;
    let mut lines = contents.lines().map(|l| l.to_string()).peekable();

    while let Some(raw) = lines.next() {
        let line = raw.trim();
        if line.starts_with("[[connect_to]]") {
            if in_block && !current.id.is_empty() {
                connects.push(current);
                current = TopologyConnectTo::default();
            }
            in_block = true;
            continue;
        }
        ...
        if line.starts_with("id") {
            if let Some(idx) = line.find('=') {
                ...
                current.id = value.to_string();
            }
            continue;
        }
        if line.starts_with("targets") {
            let text = parse_list_lines(line, &mut lines);
            current.targets = parse_string_list(&text);
            continue;
        }
        if line.starts_with("capacitys") {
            let text = parse_list_lines(line, &mut lines);
            current.capacities = parse_u128_list(&text);
            continue;
        }
    }
...
#[tokio::test]
#[ignore]
async fn test_build_network_from_topology_file() {
    init_tracing();

    let path = env::var("TOPOLOGY_PATH")
        .unwrap_or_else(|_| "/Users/yukang/Downloads/network.txt".to_string());
    let contents = match fs::read_to_string(&path) {
        Ok(data) => data,
        Err(err) => {
            eprintln!("topology file not found: {} (err: {:?})", path, err);
            return;
        }
    };

    let allow_types = env::var("TOPOLOGY_INCLUDE_TYPES")
        .unwrap_or_else(|_| "core,relay".to_string())
        .split(',')
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty())
        .collect::<Vec<_>>();

    let max_nodes = env::var("TOPOLOGY_MAX_NODES")
        .ok()
        .and_then(|v| v.parse::<usize>().ok());
    let max_edges = env::var("TOPOLOGY_MAX_EDGES")
        .ok()
        .and_then(|v| v.parse::<usize>().ok());

    let connects = parse_connect_to_blocks(&contents);
    let mut node_set: HashSet<String> = HashSet::new();
    for c in &connects {
        if !should_include_node(&c.id, &allow_types) {
            continue;
        }
        node_set.insert(c.id.clone());
        for t in &c.targets {
            if should_include_node(t, &allow_types) {
    ...
    nodes.sort_by_key(|id| {
        let (node_type, index) = node_type_and_index(id);
        (node_type_order(&node_type), index, id.clone())
    });
    ...
    let mut edges_map: BTreeMap<(usize, usize), u128> = BTreeMap::new();
    for c in &connects {
        let Some(&from_idx) = node_index_map.get(&c.id) else {
            continue;
        };
        for (pos, target) in c.targets.iter().enumerate() {
            let Some(&to_idx) = node_index_map.get(target) else {
                continue;
            };
            if from_idx == to_idx {
                continue;
            }
            let cap = c.capacities.get(pos).copied().unwrap_or(1000);
            let amount = capacity_to_amount(cap);
    ...
    let edges = edges_map
        .into_iter()
        .map(|((a, b), amount)| ((a, b), (amount, amount)))
        .collect::<Vec<_>>();

    eprintln!(
        "topology build: nodes={}, edges={} (types={:?})",
        nodes.len(),
        edges.len(),
        allow_types
    );

    let (_nodes, _channels) = create_n_nodes_network(&edges, nodes.len()).await;
}
