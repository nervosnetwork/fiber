...
#[cfg_attr(not(target_arch = "wasm32"), test)]
#[cfg_attr(target_arch = "wasm32", wasm_bindgen_test::wasm_bindgen_test)]
fn test_history_opposite_direction_success_does_not_relax_balanced_fail() {
    let mock = MockHistory::new();
    let mut history = mock.history;
    ...
    let mut fail_result = InternalResult::default();
    fail_result.fail_pair_balanced(&reverse_route, 1);
    history.apply_internal_result(fail_result);
    ...
    let mut success_result = InternalResult::default();
    success_result.succeed_range_pairs(&forward_route, 0, 1);
    history.apply_internal_result(success_result);

    let (c_to_d_direction, d_to_c_direction) = output_direction(node_c, node_d);
    assert!(matches!(
        history.get_result(&channel_outpoint, c_to_d_direction),
        Some(TimedResult {
            fail_time: 0,
            fail_amount: 0,
            success_amount,
            success_time,
        }) if *success_amount == amount && *success_time != 0
    ));
    assert!(matches!(
        history.get_result(&channel_outpoint, d_to_c_direction),
        Some(TimedResult {
            fail_time,
            fail_amount,
            success_time: 0,
            success_amount: 0,
        }) if *fail_amount == amount && *fail_time != 0
    ));

    assert_eq!(
        history.eval_probability(node_c, node_d, &channel_outpoint, amount, capacity),
        1.0
    );
    assert_eq!(
        history.eval_probability(node_d, node_c, &channel_outpoint, amount, capacity),
        0.0
    );
}
