...
pub static BLOCK_TIMESTAMP_CONTEXT: OnceCell<RwLock<BlockTimestampContext>> = OnceCell::new();
...
pub async fn get_block_timestamp(block: H256) -> u64 {
    let context = BLOCK_TIMESTAMP_CONTEXT.get_or_init(|| RwLock::new(Default::default()));
    ...
    async fn handle(
        ...
        match message {
            ...
            SendTx(tx, reply_port) => {
                ...
                let (tx_status, result) = match tx.input_pts_iter().find(|input| {
                    state
                        .cell_status
                        .get(input)
