...
pub enum InFlightCkbTxKind {
    ...
    Commitment(Hash256),
    /// Closing(peer_id, channel_id)
    Closing(PeerId, Hash256),
}
...
pub enum InternalMessage {
    Start,
    /// Send the transaction to CKB repeatlly.
    SendTx,
...
impl InFlightCkbTxActor {
    ...
    async fn report_tracing_result(
        ...
        let message = match (self.tx_kind.clone(), result.tx_status) {
            ...
            (InFlightCkbTxKind::Commitment(channel_id), TxStatus::Committed(..)) => {
                tracing::info!("Commitment transaction {:?} confirmed", self.tx_hash);
                NetworkActorEvent::CommitmentTransactionConfirmed(self.tx_hash, channel_id)
            }
            (InFlightCkbTxKind::Commitment(channel_id), status) => {
                tracing::error!(
                    "Commitment transaction {:?} failed to be confirmed with final status {:?}",
                    self.tx_hash,
                    status
                );
                NetworkActorEvent::CommitmentTransactionFailed(channel_id, self.tx_hash.into())
            }
            (InFlightCkbTxKind::Closing(peer_id, channel_id), TxStatus::Committed(..)) => {
            ...
            (InFlightCkbTxKind::Closing(peer_id, channel_id), status) => {
                tracing::error!(
