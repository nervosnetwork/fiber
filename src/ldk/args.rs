...
pub(crate) fn parse_startup_args() -> Result<LdkConfig, ()> {
    ...
    Ok(LdkConfig {
        ...
        ldk_announced_node_name: super::config::AnnouncedNodeName(ldk_announced_node_name),
...
mod rpc_auth_tests {
    ...
    fn test_parse_env_file_success() {
        ...
        assert_eq!(
            env_file_map.get(BITCOIND_RPC_USER_KEY).unwrap(),
            EXPECTED_USER
        );
        assert_eq!(
            env_file_map.get(BITCOIND_RPC_PASSWORD_KEY).unwrap(),
            EXPECTED_PASSWORD
        );
    }
