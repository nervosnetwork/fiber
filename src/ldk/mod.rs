...
pub async fn start_ldk(config: LdkConfig) {
    ...
    if bitcoind_chain
        != match config.bitcoin_network.0 {
        ...
        }
    {
        println!(
            ...
            config.bitcoin_network.0, bitcoind_chain
        );
        return;
    }
    ...
    let (channel_manager_blockhash, channel_manager) = {
        if let Ok(mut f) = fs::File::open(ldk_data_dir.join("manager")) {
            ...
            let chain_params = ChainParameters {
                network: config.bitcoin_network.0,
    ...
    let network = config.bitcoin_network.0;
    tokio::spawn(async move {
    ...
    let network = config.bitcoin_network.0;
    let announced_node_name = config.announced_node_name.0;
    let announced_listen_addr = config.announced_listen_addr;
    tokio::spawn(async move {
