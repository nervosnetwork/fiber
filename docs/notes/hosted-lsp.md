...
- one public trampoline node, Public T, which participates in the public Fiber
  network;
- multiple private Fiber tenant nodes, U1, U2, and so on, each with an
  independent node identity and database;
- one LSP payment delivery manager, which persists and resumes incoming hosted
  payments.

The implementation deliberately keeps Public T and every tenant as separate
Fiber node identities. A private channel connects each tenant directly to
Public T. Tenant runtimes disable gossip synchronization and are started on
demand, but they otherwise run the normal Fiber network and channel actors.
...
flowchart TB
    ...
    subgraph LSP["Multi-tenant Fiber LSP service"]
        ...
        subgraph R["Hosted tenant runtimes"]
            direction LR
            U1["Tenant U1<br/>independent node identity and store"]
            U2["Tenant U2<br/>independent node identity and store"]
            U3["Tenant U3<br/>independent node identity and store"]
        end
        ...
        U1 <-->|"private channel U1-T"| T
        U2 <-->|"private channel U2-T"| T
        U3 <-->|"private channel U3-T"| T
    end
...
tenant's local Fiber key. A production deployment that requires non-custodial
keys must add the remote signer wire protocol before treating this boundary as
non-custodial.
...
- Public T and tenant node identities;
- payment hash;
...
The default requested buffer duration is 15 minutes and the protocol maximum
is one hour. The hint is intentionally not embedded in the invoice encoding or
the trampoline onion payload. A wallet distributes the invoice and sidecar
together, verifies both signatures, and uses the single trampoline hop in the
hint as Public T.
...
$BASE_DIR/lsp/tenants/<id>  isolated tenant Fiber stores and keys
```
