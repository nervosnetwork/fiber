...
> **Note:** This is a legacy v0.7.x to v0.8.0 migration guide. Since v0.9.0,
> supported storage migrations are integrated into the `fnn` startup process,
> and release bundles no longer include a standalone `fnn-migrate` binary.
...
Use the `fnn-migrate` binary from the v0.8.x release that matches this legacy
upgrade path.
